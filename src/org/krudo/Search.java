...
{        
    ...
    private int abrun(int d, int a, int b, final PV pv)
    {
        ...
        // convene for transposition object
        Transpos t = TT.pick_max(node.phk, d, a, b);
        ...
        {            
            ...
            // hard cut-off
            if (s >= b) { a = b; t.hard(b); break; }
            
            // soft alfa-cut-off 
            if (s > a) { pv.cat(new_pv, m, i); a = s; t.soft(a, m, i); }
        } 

        //
        TT.save(node.phk, t, a);
    ...
    private int abmax(final int d, int a, int b, final PV pv, final int nt) 
    {
        ...
        //
        if (TT.test_max(node.phk, d, a, b)) {
            return TT.eval_max(node.phk, a, b, pv);
        }
    ...
    private int abmin(final int d, int a, int b, final PV pv, final int nt)
    {
        ...
        //
        if (TT.test_min(node.phk, d, a, b)) {
            return TT.eval_min(node.phk, a, b, pv);
        }
        ...
        // tt exit flags
        Transpos t = TT.pick_min(node.phk, d, a, b);
        ...
        {                 
            ...
            // hard cut-off
            if (s <= a) { b = a; t.hard(a); break; }
                                                
            // soft cut-off
            if (s < b) { pv.cat(new_pv, m, i); b = s; t.soft(b, m, i); }
        } 

        //
        TT.save(node.phk, t, b);
    ...
    // quiescence get_max search routine
    ...
    // quiescence get_min search routine
