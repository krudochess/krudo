...
public final class Node 
{
    ...
    // do a move placed into line-stack select by offset
    public final void domove(
        final Line line,
        final int offset
    ) {
        // loop throu moves
        for (int i = offset; i < line.i; i++)
        {
            domove(
                line.s[i],
                line.v[i],
    ...
    // do a move placed into pv-stack select by offset
    public final void domove(
        final PV pv,
        final int offset
    ) {
        // loop throu moves
        for (int i = offset; i < pv.i; i++)
        {
            domove(
                pv.s[i],
                pv.v[i],
    ...
    private void domove(final int s, final int v, final int k)
    {
        ...
        if (p == O) {
            ...
            print(desc(this));
            print(desc(this.L));
            print(desc(this.legals));
            //Legals.del(this.phk);
            legals();
            print(desc(this.legals));
            Zobrist.hash_node(this);
            print(hex(this.phk));
        }
